...
- `packages/tui/src/dashboard-actions.ts` and `packages/tui/src/dashboard-navigation.ts` now own the first extracted pure session-center/dashboard clusters (action catalogs, session-center model assembly, keyboard navigation, shortcuts, approval input, and work-pane navigation decisions), while `packages/tui/src/index.tsx` re-exports those seams instead of regrowing the logic inline.
- `packages/tui/src/dashboard-render.tsx` now owns the embedded/managed Dashboard prop-assembly helpers (`createEmbeddedWorkShellDashboardProps(...)`, `createEmbeddedWorkShellPaneDashboardProps(...)`, `createManagedWorkShellDashboardProps(...)`), further shrinking `packages/tui/src/index.tsx` toward a render-entry/component shell instead of a mixed helper monolith.
...
- `node --conditions=source --import tsx --test tests/contracts/tui-dashboard.contract.test.mjs`
  ...
  - re-verified after extracting dashboard/session-center action and navigation ownership into `packages/tui/src/dashboard-actions.ts` and `packages/tui/src/dashboard-navigation.ts`
...
- `npm run lint && npm run check && node --conditions=source --import tsx --test tests/contracts/tui-dashboard.contract.test.mjs tests/contracts/tui-work-shell.contract.test.mjs tests/contracts/tui-session-center.contract.test.mjs tests/work/repl.test.mjs tests/work/work-runtime.test.mjs tests/contracts/unclecode-cli.contract.test.mjs tests/commands/work-forwarding.test.mjs`
  - verified after extracting `dashboard-actions.ts` / `dashboard-navigation.ts` and re-exporting those seams from `packages/tui/src/index.tsx`
  - re-verified after extracting `dashboard-render.tsx` and moving embedded/managed Dashboard prop assembly out of `packages/tui/src/index.tsx`
